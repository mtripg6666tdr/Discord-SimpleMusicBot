...
class SystemInfo extends _1.BaseCommand {
    constructor() {
        super({
    ...
    getPackageVersion(mod) {
        ...
        try {
            ...
            const fsModSuffix = mod.replace(/\//g, path_1.default.sep);
            for (let i = 0; i < 5; i++) {
                packageRootPath = path_1.default.join(packageRootPath, "..");
                if (packageRootPath.endsWith(fsModSuffix)) {
                    break;
                }
            }
            return require(path_1.default.join(packageRootPath, "package.json")).version;
        }
        catch { /* empty */ }
        return "unknown";
    }
    async run(message, context) {
        ...
        if (context.args.includes("basic") || context.args.length === 0) {
            ...
            embeds.push(new helper_1.MessageEmbedBuilder()
                ...
                .addField(t("commands:log.persistentCache"), `\`${await context.bot.cache.getPersistentCacheSize()
                .then(size => (0, system_1.getMBytes)(size))
                .catch(() => "unknown")}MB\``, true)
        ...
        if (config.isBotAdmin(message.member.id) && context.args.includes("servers") || context.args.length === 0) {
            embeds.push(new helper_1.MessageEmbedBuilder()
                ...
                .setDescription(`**${t("commands:log.guildName")}(NSFW LEVEL,ID)**\r\n`
        ...
        if (config.isBotAdmin(message.member.id) && (context.args[0] === "server" && context.args[1] && context.client.guilds.has(context.args[1]))) {
            ...
            embeds.push(new helper_1.MessageEmbedBuilder()
                ...
                .addField(t("commands:log.liveStream"), data?.player.currentAudioInfo?.isYouTube() && data?.player.currentAudioInfo.isLiveStream ? t("yes") : t("no"), true)
                .setThumbnail(target.iconURL())
                .toOceanic());
        }
        if (context.args.includes("cpu") || context.args.length === 0) {
            // Process CPU Info
            const cpuInfoEmbed = new helper_1.MessageEmbedBuilder();
            cpuInfoEmbed.setColor((0, color_1.getColor)("UPTIME")).setTitle("CPU Info");
            const cpus = os.cpus();
            for (let i = 0; i < cpus.length; i++) {
                const all = cpus[i].times.user + cpus[i].times.sys + cpus[i].times.nice + cpus[i].times.irq + cpus[i].times.idle;
                cpuInfoEmbed.addField("CPU" + (i + 1), [
                    `Model: \`${cpus[i].model}\``,
        ...
        if (context.args.includes("mem") || context.args.length === 0) {
            ...
            embeds.push(new helper_1.MessageEmbedBuilder()
                ...
                .addField("Total Memory", "Total: `" + memory.total + "MB`\r\n"
                + "Used: `" + memory.used + "MB`\r\n"
                + "Free: `" + memory.free + "MB`\r\n"
                + "Usage: `" + memory.usage + "%`", true)
                .addField("Main Process Memory", "RSS: `" + rss + "MB`\r\n"
                ...
                + "External: `" + ext + "MB`\r\n"
                + "Total: `" + Util.getPercentage(rss + ext, memory.total) + "%`", true)
                .toOceanic());
        }
        if (embeds.length > 0) {
