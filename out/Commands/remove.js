...
class Rm extends _1.BaseCommand {
    constructor() {
        super({
    ...
    async run(message, context) {
        ...
        for (let i = 0; i < dels.length; i++) {
            ...
            if (Util_1.discordUtil.users.isDJ(message.member, context)
                || item.additionalInfo.addedBy.userId === message.member.id
                || !Util_1.discordUtil.channels.getVoiceMember(context)?.has(item.additionalInfo.addedBy.userId)
                || Util_1.discordUtil.channels.isOnlyListener(message.member, context)
                || Util_1.discordUtil.users.isPrivileged(message.member)) {
                // 権限等を確認して削除できるものなら削除
                q.removeAt(dels[i]);
