...
let Bgm = (() => {
    ...
    let _run_decorators;
    return _a = class Bgm extends _classSuper {
            constructor() {
                super({
                    alias: ["bgm", "study"],
                    unlist: false,
            ...
            async run(message, context) {
                ...
                const searchPanel = context.server.searchPanel.create(message, t("commands:bgm.listOfPresetBGM"), true);
                if (!searchPanel) {
                    return;
                }
                await searchPanel.consumeSearchResult(Promise.resolve(bgms), items => items.map(item => ({
                    title: item.title,
