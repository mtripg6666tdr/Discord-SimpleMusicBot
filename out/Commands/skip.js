...
class Skip extends _1.BaseCommand {
    constructor() {
        super({
    ...
    async run(message, context) {
        ...
        try {
            ...
            const members = Util_1.discordUtil.channels.getVoiceMember(context);
            context.server.updateBoundChannel(message);
            if (item.additionalInfo.addedBy.userId !== message.member.id
                && !Util_1.discordUtil.users.isDJ(message.member, context)
            ...
            await message.reply({
                content: `${context.includeMention
                    ? `<@${message.member.id}> `
