...
const logger = log4js_1.default.getLogger("Entry");
...
const bot = new bot_1.MusicBot(process.env.TOKEN, Boolean(config.maintenance));
let server = null;
// Webサーバーのインスタンス化
if (config.webserver) {
    server = (0, server_1.createServer)(bot.client, Number(process.env.PORT) || 8081);
}
else {
...
if (config.debug) {
    ...
    process.on("uncaughtException", async (error) => {
        ...
        logger.fatal(error);
        const err = await new Promise(resolve => log4js_1.default.shutdown(resolve));
...
const onTerminated = async function (code) {
    if (terminating)
        return;
    terminating = true;
...
(0, i18n_1.initLocalization)(config.debug, config.defaultLanguage).then(() => {
    // ボット開始
    bot.run();
});
async function reportError(err) {
    ...
    try {
        await bot.client.rest.channels.createMessage(config.errorChannel, {
            content: (0, Util_1.stringifyObject)(err),
        }).catch(() => { });
    }
    catch (e) {
        logger.error(e);
