...
exports.onVoiceChannelLeave = onVoiceChannelLeave;
const tslib_1 = require("tslib");
const i18next_1 = tslib_1.__importDefault(require("i18next"));
const queueManagerWithBGM_1 = require("../Component/queueManagerWithBGM");
const config_1 = require("../config");
const config = (0, config_1.getConfig)();
async function onVoiceChannelLeave(member, oldChannel) {
    ...
    if (member.id === this._client.user.id) {
        ...
        await this._client.rest.channels.createMessage(server.boundTextChannel, {
            content: `:postbox: ${i18next_1.default.t("disconnected", { lng: server.locale })}`,
        ...
        if (server.queue instanceof queueManagerWithBGM_1.QueueManagerWithBgm && server.queue.isBGM) {
            await server.player.disconnect().catch(this.logger.error);
        }
            ...
            if (server.player.isPlaying
                ...
                if (server.player.isPlaying && !server.player.isPaused) {
                    ...
                    if (server.boundTextChannel) {
                        await this._client.rest.channels.createMessage(server.boundTextChannel, {
                            content: `:pause_button:${i18next_1.default.t("autoPaused", { lng: server.locale })}`,
                        }).catch(this.logger.error);
                    }
                }
                const timer = setTimeout(() => {
                    ...
                    this._client.rest.channels.createMessage(server.boundTextChannel, {
                        content: `:postbox: ${i18next_1.default.t("autoDisconnect", { lng: server.locale })}`,
                    }).catch(this.logger.error);
