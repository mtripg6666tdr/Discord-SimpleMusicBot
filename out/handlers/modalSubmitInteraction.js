...
async function handleModalSubmitInteraction(server, interaction) {
    if (!interaction.inCachedGuildChannel())
        return;
    this.logger.info("received modalsubmit interaction");
    const targetCommand = commandManager_1.CommandManager.instance.resolve(interaction.data.customID);
    await targetCommand?.handleModalSubmitInteraction(interaction, server);
}
//# sourceMappingURL=modalSubmitInteraction.js.map