...
### `cacheLevel` ("memory" | "persistent" | "full")
  ...
  `"memory"`、`"persistent"`、`"full"`の順にキャッシュレベルが高くなり、キャッシュレベルが高いほど、再生が高速化されます。

:::info

各キャッシュレベルでのキャッシュ動作の違いについては、以下の表のようになっています。

|                                |`"memory"`|`"persistent"`|`"full"`|
|--------------------------------|:--------:|:------------:|:-----:|
|メモリ上のメタ情報のキャッシュ     |〇        |〇            |〇      |
|メモリ上のストリーム情報のキャッシュ|〇        |〇            |〇      |
|ディスク上のメタ情報のキャッシュ   |          |〇            |〇      |
|ディスク上の音声情報のキャッシュ   |          |              |〇      |
