...
export const FallBackNotice = "現在、通常の方法で情報を取得できなかったため、代替ライブラリにフォールバックして取得しました。処理に時間がかかる、突然終了するなど、正常なオペレーションができない場合があります。再生できない場合でも、サポートの対象外です。問題が発生した場合、`リセット`コマンドを使用すると、サーバーのデータをすべて初期化することができます。";
