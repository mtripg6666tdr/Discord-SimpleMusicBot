...
export abstract class MusicBotBase extends LogEmitter {
  // クライアントの初期化
  protected abstract readonly _client: discord.Client;
  ...
  logGeneralInfo() {
    ...
    Util.logger.log(
      `[Tick] [System] Free:${Math.floor(memory.free)}MB; Total:${Math.floor(
        memory.total,
      )}MB; Usage:${memory.usage}%`,
    );
    const nMem = process.memoryUsage();
    const rss = Util.system.GetMBytes(nMem.rss);
    const ext = Util.system.GetMBytes(nMem.external);
    Util.logger.log(
      `[Tick] [Main] Memory RSS: ${rss}MB, Heap total: ${Util.system.GetMBytes(
        nMem.heapTotal,
      )}MB, Total: ${Util.math.GetPercentage(rss + ext, memory.total)}%`,
    );
  }
  ...
  override on<T extends keyof BotBaseEvents>(
    eventName: T,
    listener: (...args: BotBaseEvents[T]) => void,
  ) {
    return super.on(eventName, listener);
  }

  override once<T extends keyof BotBaseEvents>(
    eventName: T,
    listener: (...args: BotBaseEvents[T]) => void,
  ) {
    return super.on(eventName, listener);
  ...
  override off<T extends keyof BotBaseEvents>(
    eventName: T,
    listener: (...args: BotBaseEvents[T]) => void,
  ) {
