import type { AudioSource, defaultM3u8stream } from "../AudioSource/audiosource";
import type { YouTube } from "../AudioSource/youtube";
import type { GuildVoiceInfo } from "../definition";
import type { Client, Message, StreamDispatcher, TextChannel } from "discord.js";
...
import { EventEmitterLike, FallBackNotice } from "../definition";
...
export class PlayManager extends ManagerBase {
  ...
  async Play(): Promise<PlayManager>{
    ...
    try{
      ...
      const stream: Readable|string = this.ResolveStream(rawStream);
      ...
      this.Dispatcher.on("error", async (e)=>{
        // エラーが発生したら再生できないときの関数を呼んで逃げる
        log(JSON.stringify(e), "error");
        if(this.info.boundTextChannel){
          await this.client.channels.fetch(this.info.boundTextChannel).then(async channels => {
            log("[PlayManager/" + this.info.GuildID + "]Some error occurred in StreamDispatcher", "error");
            await (channels as TextChannel).send(":tired_face:曲の再生に失敗しました...。(" + (e ? e.message ?? e : "undefined") + ")" + ((this.errorCount + 1) >= this.retryLimit ? "スキップします。" : "再試行します。"));
          });
        }
        await cantPlay();
      });
      ...
      try{
        ...
        if(t.includes("429")){
          await mes.edit(":sob:レート制限が検出されました。しばらくの間YouTubeはご利用いただけません。");
      ...
      if(this.info.boundTextChannel && ch && mes){
        await mes.edit(":tired_face:曲の再生に失敗しました...。" + ((this.errorCount + 1) >= this.retryLimit ? "スキップします。" : "再試行します。"));
        await cantPlay();
      }
    }
    return this;
  ...
  */
   ...
   * @returns this
   */
  async Pause(): Promise<PlayManager>{
    await this.info.Bot.BackupStatus();
   ...
   * @returns this
   */
  async Resume(): Promise<PlayManager>{
    await this.info.Bot.BackupStatus();
    log("[PlayManager/" + this.info.GuildID + "]Resume() called");
    this.Dispatcher?.resume();
    if(this.pausedSince !== 0) this.startTime += new Date().getTime() - this.pausedSince;
    return this;
  }
  ...
  private onDispatcherFinished(){
    ...
    setTimeout(async ()=>{
      ...
      if(this.info.Queue.length === 0){
        ...
        if(this.info.boundTextChannel){
          await this.client.channels.fetch(this.info.boundTextChannel).then(ch => {
