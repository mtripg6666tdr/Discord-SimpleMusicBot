...
export default class SystemInfo implements CommandInterface {
  ...
  async run(message: discord.Message, options: CommandArgs){
    ...
    if(message.author.id === (process.env.ADMIN_USER ?? "593758391395155978") && (options.args.includes("log") || options.args.length === 0)){
      // Process Logs
    ...
    if(options.args.includes("mem") || options.args.length === 0){
      ...
      memInfoEmbed.addField("Total Memory",
        ...
        + "Usage: `" + memory.usage + "%`"
        , true);
      const rss = GetMBytes(nMem.rss);
      const ext = GetMBytes(nMem.external);
