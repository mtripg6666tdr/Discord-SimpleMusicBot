...
export default class Play extends BaseCommand {
      ...
      argument: [
        {
          ...
          name: "keyword",
          description:
            "再生する動画のキーワードまたはURL。VCに未接続の場合接続してその曲を優先して再生します。接続中の場合はキューの末尾に追加します。一時停止中の場合はオプションは無視され、再生が再開されます。",
          required: false,
        },
      ],
  ...
  async run(message: CommandMessage, options: CommandArgs) {
    ...
    if (options.rawArgs !== "") {
      if (
        options.rawArgs.startsWith("http://") ||
        options.rawArgs.startsWith("https://")
      ) {
        ...
        try {
          const result = (await searchYouTube(options.rawArgs)).items.filter(
            it => it.type === "video",
          ) as ytsr.Video[];
          if (result.length === 0) {
            await message.reply(
              ":face_with_monocle:該当する動画が見つかりませんでした",
            );
            await msg.delete();
            return;
          }
      ...
      // URLのみのメッセージか？
      if (
        messageReference.content.startsWith("http://") ||
        messageReference.content.startsWith("https://")
      ) {
        await options.server.playFromURL(
          message,
        ...
        await options.server.playFromURL(
          message,
          messageReference.content.substring(prefixLength),
          !wasConnected,
        );
        // 添付ファイル付きか？
        ...
        await options.server.playFromURL(
          message,
          messageReference.attachments[0].url,
          !wasConnected,
        );
        // ボットのメッセージなら
        ...
        if (
          ...
          const url = embed.description.match(/^\[.+\]\((?<url>https?.+)\)/)
            ?.groups.url;
          await options.server.playFromURL(message, url, !wasConnected);
        } else {
          await message
            .reply(
              ":face_with_raised_eyebrow:返信先のメッセージに再生できるコンテンツが見つかりません",
            )
            .catch(e => Util.logger.log(e, "error"));
        }
      } else {
