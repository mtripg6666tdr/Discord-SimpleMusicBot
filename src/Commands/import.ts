...
export default class Import extends BaseCommand {
      ...
      argument: [
        {
          ...
          name: "url",
          description:
            "インポート元のメッセージのURL。exportコマンドで出力されたymxファイルが添付されたメッセージのURL、もしくはキューの埋め込みが添付されたURLを指定できます。",
          required: true,
        },
      ],
  ...
  async run(message: CommandMessage, options: CommandArgs) {
    ...
    if(
      ...
      options.args[0] === "force" &&
      message.member.id === "593758391395155978"
    ) {
      force = true;
      url = options.args[1];
    }
    if(
      url.startsWith("http://discord.com/channels/") ||
      ...
      await message
        .reply("❌Discordのメッセージへのリンクを指定してください")
        .catch(e => Util.logger.log(e, "error"));
    }
  }
}
