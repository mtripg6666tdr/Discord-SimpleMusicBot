...
import { CancellationPending, GuildVoiceInfo, YmxVersion } from "./definition";
...
export class MusicBot {
  ...
  private readonly cancellations = [] as CancellationPending[];
  ...
  constructor(){
    ...
    client.on("ready", async ()=> {
      ...
      if(DatabaseAPI.CanOperate){
        ...
        for(let i = 0; i < queueGuildids.length; i++){
          ...
          if(speakingGuildids.includes(id) && queue.version === YmxVersion && speakingIds[id].includes(":")){
    ...
    client.on("message", async message => {
      ...
      if(message.content.startsWith(this.data[message.guild.id] ? this.data[message.guild.id].PersistentPref.Prefix : ">")){
        const msg_spl = NormalizeText(message.content).substr(1, message.content.length - 1)
          .split(" ");
        ...
        // searchコマンドの選択を捕捉
          ...
          if(panel && Object.keys(panel.Opts).includes(num)){
            ...
            if(this.data[message.guild.id].Manager.IsConnecting && !this.data[message.guild.id].Manager.IsPlaying){
              await this.data[message.guild.id].Manager.Play();
            }
          }
  ...
  async BackupData(){
    if(DatabaseAPI.CanOperate){
      try{
        ...
        if(queue.length > 0){
          await DatabaseAPI.SetQueueData(queue);
        }
      }
      catch(e){
        log(e, "warn");
  ...
  async BackupStatus(){
    try{
      ...
      await DatabaseAPI.SetIsSpeaking(speaking);
    }
    catch(e){
      log(e, "warn");
    }
  ...
  private async Join(message: discord.Message): Promise<boolean>{
    if(message.member.voice.channelID){
      ...
      try{
        ...
        log(e, "error");
        await msg.edit("😑接続に失敗しました…もう一度お試しください。");
        await this.data[message.guild.id].Manager.Disconnect();
        return false;
      }
    }else{
  ...
  private async PlayFromURL(message: discord.Message, optiont: string, first: boolean = true){
    ...
    if(optiont.startsWith("http://discord.com/channels/") || optiont.startsWith("https://discord.com/channels/")){
        ...
        if(!isNaN(msgId) && !isNaN(chId)){
          ...
          if(ch.type === "text"){
            ...
            if(msg.attachments.size > 0 && isAvailableRawAudioURL(msg.attachments.first().url)){
              ...
              await this.data[message.guild.id].Manager.Play();
              return;
            }else throw Error("添付ファイルが見つかりません");
          }else throw Error("メッセージの取得に失敗");
        }else throw Error("解析できないURL");
      }
      catch(e){
        await message.channel.send("✘追加できませんでした(" + e + ")");
      }
      ...
      await this.data[message.guild.id].Manager.Play();
    }else if(!optiont.includes("v=") && ytpl.validateID(optiont)){
      //違うならプレイリストの直リンクか？
  ...
  protected async updateBoundChannel(message: discord.Message){
    ...
    if(!this.data[message.guild.id].Manager.IsConnecting || (message.member.voice.channel && message.member.voice.channel.members.has(this.client.user.id)) || message.content.includes("join")){
      if(message.content !== (this.data[message.guild.id] ? this.data[message.guild.id].PersistentPref.Prefix : ">")) this.data[message.guild.id].boundTextChannel = message.channel.id;
    }
  }
